...
describe("DateTime Helpers", () => {
  ...
  describe("toDisplayString", () => {
    it("should format date and time as localized string", () => {
      const date = new Date(2026, 4, 5, 9, 5);
      // In Paris (UTC+2), if the browser is in CEST, this is 09:05 local
      // We expect the French format
    ...
    it("should handle a specific timezone (e.g. New York)", () => {
      ...
      expect(toDisplayString(iso, "America/New_York")).toContain("08:00");
      // 12:00 UTC is 14:00 in Paris (CEST, UTC+2)
      expect(toDisplayString(iso, "Europe/Paris")).toContain("14:00");
    });
  });
});
